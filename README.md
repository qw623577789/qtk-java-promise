...
dependencies {
    ...
    annotationProcessor 'io.github.vipcxj:jasync-core:0.1.6'

    // testAnnotationProcessor 'org.projectlombok:lombok:1.18.22' 若用上lombok，要放在前面
    testAnnotationProcessor 'io.github.vipcxj:jasync-core:0.1.6'
}
```
...
- **JPromise\<?> deferResolve(Supplier\<T> deferFunc)** 包装lamda表达式(**返回同步结果**)成Promise对象，当触发``async()、block()、await()``时，才执行lamda方法触发Promise.resolve
- **JPromise\<?> deferPromiseResolve(Supplier\<JPromise\<T>> deferFunc)** 包装lamda表达式(**返回异步对象**)成Promise对象，当触发``async()、block()、await()``时，才执行lamda方法触发Promise.resolve
