...
public class Promise {
    ...
    /**
     * 【指定线程】惰性resolve模式执行异步函数，即只有真正等到触发``async()、block()、awiat()``时，才触发Promise.resolve
     * 一般用于指定Promise运行线程
    ...
    /**
     *【指定线程】惰性resolve模式执行同步函数，即只有真正等到触发``async()、block()、awiat()``时，才触发Promise.resolve
    ...
    /**
     *【当前线程】惰性resolve模式执行同步函数，即只有真正等到触发``async()、block()、awiat()``时，才触发Promise.resolve
     * 一般用于延后运行同步函数
